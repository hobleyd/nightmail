...
class OAuthCredentialsStorage {
  ...
  static const _msClientSecret = 'oauth_microsoft_client_secret';
  static const _googleClientId = 'oauth_google_client_id';
  static const _googleRedirectUri = 'oauth_google_redirect_uri';
  static const _googleClientSecret = 'oauth_google_client_secret';
  ...
  Future<OAuthCredentials> load() async {
    ...
    final credentials = OAuthCredentials(
      ...
      microsoftClientSecret: await _storage.read(key: _msClientSecret),
      googleClientId:
      ...
      googleClientSecret: await _storage.read(key: _googleClientSecret),
    );
  ...
  Future<void> save(OAuthCredentials credentials) async {
    ...
    if (credentials.microsoftClientSecret != null) {
      await _storage.write(
          key: _msClientSecret, value: credentials.microsoftClientSecret);
    } else {
      await _storage.delete(key: _msClientSecret);
    }
    await _storage.write(
        key: _googleClientId, value: credentials.googleClientId);
    await _storage.write(
        key: _googleRedirectUri, value: credentials.googleRedirectUri);
    if (credentials.googleClientSecret != null) {
      await _storage.write(
          key: _googleClientSecret, value: credentials.googleClientSecret);
    } else {
      await _storage.delete(key: _googleClientSecret);
    }
  }
}
