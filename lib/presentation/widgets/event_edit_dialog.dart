        ...
        Row(
          children: [
            ...
            if (!isAllDay) ...[
              ...
              readOnly
                  ? _TimeButton(time: startTime, onTap: () {})
                  : _TimeComboBox(
        ...
        Row(
          children: [
            ...
            if (!isAllDay) ...[
              const SizedBox(width: 6),
              readOnly
                  ? _TimeButton(time: endTime, onTap: () {})
                  : _TimeComboBox(
                      time: endTime, onChanged: onEndTimeChanged),
            ],
          ],
        ),
      ],
...
List<TimeOfDay> _timeSlotsAnchoredTo(TimeOfDay anchor) {
...
TimeOfDay? _parseTimeOfDay(String raw, TimeOfDay previous) {
...
class _TimeComboBox extends StatefulWidget {
  const _TimeComboBox({required this.time, required this.onChanged});
  final TimeOfDay time;
  final ValueChanged<TimeOfDay> onChanged;

  @override
  State<_TimeComboBox> createState() => _TimeComboBoxState();
}

class _TimeComboBoxState extends State<_TimeComboBox> {
  ...
  void initState() {
    super.initState();
    _controller = TextEditingController(text: _formatTimeOfDay(widget.time));
    _focusNode.addListener(_onFocusChanged);
  }

  @override
  void didUpdateWidget(_TimeComboBox old) {
    super.didUpdateWidget(old);
    if (!_focusNode.hasFocus && old.time != widget.time) {
      _controller.text = _formatTimeOfDay(widget.time);
    }
  }
  ...
  void _openDropdown() {
    final slots = _timeSlotsAnchoredTo(widget.time);
    setState(() {
      _daySlots = slots;
    ...
    final target = _highlightIndex >= 0 ? _highlightIndex : _nearestSlotIndex(slots);
  ...
  void _onTextChanged(String text) {
    ...
    setState(() {
      _filteredSlots = query.isEmpty
          ? _daySlots
          : _daySlots
              .where((s) => _formatTimeOfDay(s).toLowerCase().startsWith(query))
              .toList();
  ...
  void _selectSlot(TimeOfDay t) {
    _controller.text = _formatTimeOfDay(t);
    if (t != widget.time) widget.onChanged(t);
    _closeDropdown();
  }

  void _commit() {
    final parsed = _parseTimeOfDay(_controller.text, widget.time);
    if (parsed == null) {
      _controller.text = _formatTimeOfDay(widget.time);
      return;
    }
    _controller.text = _formatTimeOfDay(parsed);
  ...
  KeyEventResult _onKeyEvent(FocusNode node, KeyEvent event) {
    ...
    if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
      ...
      _controller.text = _formatTimeOfDay(_filteredSlots[_highlightIndex]);
      if (!_overlayController.isShowing) _overlayController.show();
    ...
    return Material(
      ...
      child: ConstrainedBox(
        constraints: const BoxConstraints(
            minWidth: 92, maxWidth: 92, maxHeight: 240),
