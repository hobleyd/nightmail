...
namespace flutter_inappwebview_plugin
{
  ...
  private:
    ...
    // Shared alive flag: set to false at the START of ~InAppWebView() so that
    // any pending async callbacks (DevTools, JS bridge, navigation events) that
    // fire after destruction can detect this and bail out before touching freed
    // members.  All lambdas that capture `this` must also capture a copy of
    // this shared_ptr and check `*isAlive_` as their first line.
