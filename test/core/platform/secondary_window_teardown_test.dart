...
void main() {
  final mainWindow = File('macos/Runner/MainFlutterWindow.swift').readAsStringSync();
  final webKitView =
      File('packages/html_view/macos/Classes/WebKitView.swift').readAsStringSync();

  test('a secondary window shuts its engine down when it closes', () {
    expect(mainWindow, contains('NSWindow.willCloseNotification'),
        reason: 'the only signal desktop_multi_window itself acts on');
    expect(mainWindow, contains('controller?.engine.shutDownEngine()'),
        reason: 'the NSWindow never deallocates, so the engine must be ended '
            'explicitly');
    expect(mainWindow, contains('window?.contentViewController = nil'),
        reason: 'detaching the controller is what lets the retain cycle unwind');
    expect(mainWindow, contains('DispatchQueue.main.async'),
        reason: "the notification fires inside window_manager's close handler, "
  ...
  test("the window's relay channels are forgotten with it", () {
    ...
    expect(mainWindow, contains('self?.forgetChannels(channels)'),
        reason: 'a broadcast to a dead engine logs "Invalid engine handle" '
            'for every relay it was still on');
  });

  test('the webview releases its key monitor without a destroyView', () {
    expect(webKitView, contains('deinit { dispose() }'),
        reason: 'the Dart side cannot call destroyView once its engine is gone');
  });

  test('a webview leaves with the window that hosted it', () {
    final plugin =
        File('packages/html_view/macos/Classes/HtmlViewPlugin.swift').readAsStringSync();
    expect(webKitView, contains('NSWindow.willCloseNotification'),
        reason: 'a shut-down engine keeps its plugins, so without this the '
            'WKWebView and its WebContent process outlive the window');
    expect(plugin, contains('self?.views.removeValue(forKey: id)?.dispose()'),
        reason: 'the registry entry is the last strong reference');
    expect(webKitView, contains('if disposed { return }'),
